...
### Requirement: nb-mcp-server SHALL depend on published nb-api
After the split, `nb-mcp-server` SHALL depend on `nb-api` via crates.io published version only (no path dependency).

#### Scenario: Dependency declaration
- **WHEN** a consumer reads `nb-mcp-server/Cargo.toml`
- **THEN** the `nb-api` dependency SHALL use `version = "0.1"` without a `path` field
